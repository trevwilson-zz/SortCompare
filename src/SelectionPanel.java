...
public class SelectionPanel extends JPanel {
	...
	/**
	 * Takes in arguments to indicate whether the sort should be performed and
	 * whether the initial array should be sorted randomly or set up for a worst
	 * case.
	 * 
	 * @param enabled
	 *            If true, an array will be generated and sorted
	 * @param randomCase
	 *            If true, and random array is generated. If false, a worst-case
	 *            array is generate with values in descending order
	 */
	...
	/**
	 * Generates and draws the initial state of the array and then sorts it
	 * while redrawing in real time.
	 */
	public void draw() {
		if (enabled) {

			// Wraps the entire method in a Runnable to ensure it is redrawn
			// correctly
	 ...
	 * 
	 * @param g
	 *            The Graphics context to redraw on
	 */
	public void paintComponent(Graphics g) {
		if (enabled) {
			super.paintComponent(g);
			for (int i = 0; i < 300; i++) {
				g.drawRect(i * 2, 25 + (300 - selectionArray[i]), 2,
						selectionArray[i]);
				g.fillRect(i * 2, 25 + (300 - selectionArray[i]), 2,
						selectionArray[i]);
			}
		}
	}
	 ...
	 * 
	 * @param index1
	 *            The position of the first value in the array to be swapped
	 * @param index2
	 *            The position of the second value in the array to be swapped
	 */
